...
void vehicle::damage_all( int dmg1, int dmg2, damage_type type, const point &impact )
{
    ...
    for( const vpart_reference &vp : get_all_parts() ) {
        ...
        if (distance > 1) {
            ...
            if (part_info(p).location != part_location_structure || !part_info(p).has_flag("PROTRUSION")) {
                int shock_absorber = part_with_feature(p, "SHOCK_ABSORBER", true);
                if (shock_absorber >= 0) {
                    // should be the shock absorber's bonus but hardcoded it at 10 for now
                    net_dmg = std::max(0, net_dmg - 10);
                }
            }
